...
---------------------------*/
var myCities = [  //NAME AND BOUNDS OF CITIES -97.938383,30.098659,-97.56842,30.516863
...
,giturl = "https://github.com/psmyth2/parkamenities" //Only change this if you want to link to a fork you made, otherwise you can leave the link to the original repo
,twiturl = "https://twitter.com/PatrickGSmyth" //Links to my twit acct, change it if you want or remove twitter link altogether
,myPath = "https://psmyth2.carto.com/api/v2"; //this is the root path to your cartoDB instance with the v2 api param
...
,parkRate = 999//no data value //***park rating
,parkVisit = 999//no data value //***park visit
,hStyle = {
    "stroke":true,
...
function go(){
  ...
  map.setView([30.26715, -97.74306],13)
  ...
  map.addLayer(drawnItems);
	...
	//make the nav and city buttons-------City buttons Y'All!
  ...
  //CHECK IF Neighborhood has a name
    ...
    //CHECK IF Park has a name
    if (!notEmpty(document.getElementById('parkName'))){
    ...
    $(".park-group > button.btn").removeClass('active');
    ...
    showAlert("Done!","Your amenity has been added! Draw/place more amenities, or take a look at what has been added so far by clicking 'View Maps'.");
  });
...
var loadHoods = function(){
  ...
  cartodb.createLayer(map, {
    ...
    interactivity: 'cartodb_id,nbrhd_name, description',
...
var hoodClickGetter = function(ll){
  getJSON.abort();
  getJSON = $.ajax(
    {url:myPath+"/sql?q=SELECT nbrhd_name FROM "+tblName+" WHERE ST_Intersects( the_geom, ST_SetSRID((ST_POINT("+ll.lng+", "+ll.lat+")) , 4326)) AND flag = false GROUP BY nbrhd_name ORDER BY nbrhd_name ASC",
    crossDomain:true,
      dataType:"jsonp",
      error: function (xhr, text_status, error_thrown) {
        ...
        for(var i = 0;i<data.rows.length;i++){
          ...
          if(boo){pgArr+=data.rows[i].nbrhd_name+',';unique = true;}
          console.log(data.rows[i].nbrhd_name);
        }
        if(!unique){
          return;
        }
...
var getNewHoods = function (arr){//takes array of neighborhoood names, gets and draws geojson
 ...
 getJSON = $.ajax(
    {url:myPath+"/sql?q=SELECT nbrhd_name,COUNT(nbrhd_name),array_agg(description)as description,array_agg(loved)as loved,array_agg(cartodb_id) as cartodb_id,array_agg(ST_AsGeoJSON(the_geom)) as the_geom FROM park_amenities WHERE nbrhd_name = ANY('"+arr+"') AND flag = false AND ST_Intersects(ST_Envelope(the_geom), ST_MakeEnvelope("+left+","+bottom+","+right+","+top+",4326)) GROUP BY nbrhd_name ORDER BY nbrhd_name ASC",
      crossDomain:true,
      dataType:"jsonp",
      error: function (xhr, text_status, error_thrown) {
        console.log(text_status)
...
var clearSomeHoods = function(cnt){
  for(var i=0;i<cnt;i++){
    for(var h=0;h<highlightHoods.length;h++){
      if(highlightHoods[h].nbrhd_name != ''){
  ...
  $.ajax(
    {url:myPath+"/sql?q=SELECT nbrhd_name,COUNT(nbrhd_name) FROM "+tblName+" WHERE flag = false GROUP BY nbrhd_name ORDER BY nbrhd_name ASC",
      ...
      $.each(data.rows, function() {
        if ( newARR.indexOf( this.nbrhd_name) == -1 )
      newARR.push(this.nbrhd_name);//this.name
    });
...
function showInstructions(){
  ...
  var title = 'Sketch your first park amenity!',
    text = 'To get started, zoom to your area of interest, then ' + action + ' the <b>amenity SHAPE</b> or <b>amenity POINT</b> button.',
...
function showDrawingInstructions(){
  ...
  if ( geomType == "poly" ){
    ...
    var title = 'Now trace the amenity or feature',
      text = action + ' to draw the shape of the amenity. You won\'t be able to move the map while drawing, but don\'t worry, you can make changes after you finish.',
...
function showEditingInstructions(){
  ...
  if ( geomType == "poly" ){
    ...
    var title = 'Looking good!',
      text = 'You can now adjust the shape of the amenity. Drag the white squares to change the shape, or ' + action + ' them to delete corners. When you are satisfied with the shape, ' + action + ' <b>Save Amenity</b> to submit it.',
      src = 'img/instructions3_poly.gif';
    showAlert( title, text, src );
  } else {
    if ( L.Browser.touch ) return;
    var title = 'Looking good!',
      text = 'You can click and drag the marker to edit its location. When you are satisfied, click <b>Save Amenity</b> to submit it.',
